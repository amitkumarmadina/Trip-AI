...
import { Loader2, Sparkles, Plane, ArrowRight, Eye, EyeOff, User, Mail, Lock } from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/register")({
  head: () => ({
    meta: [
      { title: "Sign Up — Trip AI" },
      { name: "description", content: "Create your Trip AI account." },
    ],
  }),
...
function RegisterPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      register(data.token, { id: data.id, name: data.name, email: data.email });
  ...
  return (
    <div className="relative min-h-screen bg-[#070b13] text-foreground overflow-hidden flex flex-col justify-between select-none">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 md:py-20 relative z-10 w-full flex items-center justify-center grow">
        <div className="glass-panel w-full rounded-3xl border border-white/5 overflow-hidden shadow-2xl grid grid-cols-1 md:grid-cols-2 min-h-[550px] bg-[#0b1220]/80 backdrop-blur-xl">
          ...
          <div className="relative hidden md:block overflow-hidden min-h-full">
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-9 text-left space-y-3.5">
              ...
              <p className="text-[13px] text-white/70 leading-relaxed font-outfit font-semibold">
                Sign up to TripAI and unlock the power of AI to plan your perfect trip.
              </p>
            </div>
          </div>
          ...
          <div className="p-8 sm:p-12 flex flex-col justify-center text-left bg-[#0B1220]/50 relative">
            <div className="space-y-6">
              <div>
                <h1 className="text-[25px] font-black text-white tracking-tight font-outfit">Create your account</h1>
                <p className="text-[12px] text-[#94A3B8] font-bold mt-1 font-outfit uppercase tracking-wider">
                  Sign up to start your adventures
                </p>
              </div>
              ...
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label htmlFor="name" className="text-[10px] font-black uppercase tracking-wider text-[#94A3B8] font-outfit">
                    Your name
                  </label>
              ...
              {/* Redirect to Login */}
              <div className="text-center text-[12px] text-[#94A3B8] pt-4.5 border-t border-white/5 font-outfit font-bold">
                Already have an account?{" "}
                <Link to="/login" className="text-[#10B981] font-extrabold hover:underline">
                  Sign in here
                </Link>
              </div>
            </div>
